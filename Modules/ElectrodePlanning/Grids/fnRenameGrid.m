...
strOldName = g_strctModule.m_astrctChambers(g_strctModule.m_iCurrChamber).m_astrctGrids(iSelectedGrid).m_strName;
answer=inputdlg({'New Chamber Name:'},'Change Volume Name',1,{strOldName});
if isempty(answer)
    return;
end
g_strctModule.m_astrctChambers(g_strctModule.m_iCurrChamber).m_astrctGrids(iSelectedGrid).m_strName = answer{1};
fnUpdateGridList();
return;
