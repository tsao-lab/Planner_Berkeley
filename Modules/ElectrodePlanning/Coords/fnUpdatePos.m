...
if ~isempty(strctMouseOp.m_pt2fPos)
    ...
    if 1% strcmpi(g_strctModule.m_strMouseMode,'QueryAtlas')
        if exist('pt3fPosIn3DSpace', 'var') && all(size(pt3fPosIn3DSpace)==[4 1])
            pt3fAtlas = g_strctApp.m_strctAtlasNew.m_strctMRI.vox2ras0 \ pt3fPosIn3DSpace;
            index = fndllFastInterp3(g_strctApp.m_strctAtlasNew.m_strctMRI.vol, ...
                pt3fAtlas(1), pt3fAtlas(2), pt3fAtlas(3), 1);
            idx = find(g_strctApp.m_strctAtlasNew.m_tableRegions.label==index, 1);
        else
            idx = [];
        end
        if isempty(idx)
            ...
            strAtlasRegion = g_strctApp.m_strctAtlasNew.m_tableRegions.name{idx};
        end
