...
if g_strctModule.m_bFirstInvalidate && ~ismember(strCallback, ...
        {'LoadSession', 'LoadAnatVol', 'SetNormalMode', 'SetAtlasMode', 'SetChamberMode', 'ModuleSwitch'})
    return
end
...
switch lower(strctKey.Key)
    ...
    case 'space'
        g_strctModule.m_strctGUIOptions.m_bImageSeries = ...
            ~g_strctModule.m_strctGUIOptions.m_bImageSeries;
        fnInvalidate();
    case 'period'
        iActiveSeries = get(g_strctModule.m_strctPanel.m_hImageSeriesList,'value');
        g_strctModule.m_astrctImageSeries(iActiveSeries).m_fOpacity = ...
            min(1,g_strctModule.m_astrctImageSeries(iActiveSeries).m_fOpacity + 0.1);
        fnInvalidate();
    case 'comma'
        iActiveSeries = get(g_strctModule.m_strctPanel.m_hImageSeriesList,'value');
        g_strctModule.m_astrctImageSeries(iActiveSeries).m_fOpacity = ...
            max(0,g_strctModule.m_astrctImageSeries(iActiveSeries).m_fOpacity - 0.1);
        fnInvalidate();
end
