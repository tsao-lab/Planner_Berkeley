...
strctTarget.m_pt3fPosition = pt3fPosInVol;
strctTarget.m_strName = sprintf('Unknown (AP %.2f ML %.2f DV %.2f)', ...
...
if ~isfield(g_strctModule, 'm_astrctTargets') || ...
    ...
    g_strctModule.m_astrctTargets = strctTarget;
else
    iNumTargets = length(g_strctModule.m_astrctTargets);    
    g_strctModule.m_astrctTargets(iNumTargets+1) = strctTarget;
end
fnUpdateTargetList();
