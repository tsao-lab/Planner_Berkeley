...
if ~isfield(g_strctModule.m_acAnatVol{g_strctModule.m_iCurrAnatVol},'m_astrctROIs') || ...
        isempty(g_strctModule.m_astrctROIs)
     g_strctModule.m_astrctROIs = strctROI;
else
    iNumROI = length(g_strctModule.m_astrctROIs);    
    ...
    g_strctModule.m_astrctROIs(iNumROI+1) = strctROI;
end
iNewROIIndex = length(g_strctModule.m_astrctROIs);
fnUpdateROIList();
